...
export default function OurBranches() {
  ...
  const branches = [
    ...
    {
      barnchName: "Derabasi",
      address: "#1248, first floor, near Punjab properties, Barwala Road, Derabassi",
    },
    {
      barnchName: "Ghanaur",
      address: "OIC GLOBAL GHANAUR (Near Maruti Agency, First Floor Rainbow Cafe)",
    },
    {
      barnchName: "Doraha",
    ...
    {
      barnchName: "Pehowa",
      address: "OIC Global Pehowa (1st Floor, Super Market Pehowa, Near Bus Stand)",
    },
  ];

  return (
    <div className="px-5  md:px-16 xl:px-32 pt-8 md:py-12 xl:pb-0 xl:pt-16">
      <h4 className="font-bold text-xl sm:text-2xl md:text-3xl mb-4">Our Branches</h4>
          ...
          return (
            <div
              ...
              className={`h-[240px] sm:h-[260px] p-4 rounded-lg shadow-md cursor-pointer transition-all duration-300 flex flex-col items-center justify-center gap-y-3
                ${isActive ? "bg-[#D92024] text-white" : "bg-white text-black border-b-4 border-[#D92024]"}
              `}
            >
              <button
                className={`text-sm sm:text-base md:text-lg font-semibold px-4 py-2 rounded-full transition-all duration-300
                  ${isActive ? "bg-white text-[#D92024]" : "border-2 border-[#D92024] text-[#D92024]"}
                `}
              >
                {elm.barnchName}
              </button>
